...
int P1_shellTask(int argc, char* argv[]) {
	...
	sigAction(shellSigIntHandler, mySIGINT);
	...
	while (1) {
...

		{
			...
			// parse input string
			char splitChar = ' ';
			while ((sp = strchr(sp, splitChar)) && newArgc < MAX_ARGS) {
				if (splitChar == '"') {
					splitChar = ' ';
					*sp++ = 0;
				}
		...
		for (found = i = 0; i < NUM_COMMANDS; i++) {
			if (!STRCMPNOCASE(newArgv[0], commands[i]->command) || !STRCMPNOCASE(newArgv[0], commands[i]->shortcut))
			{
				found = TRUE;

				if (newArgc > 1 && !strcmp(newArgv[newArgc-1], "&")) // execute background
				{
...
int P1_add(int argc, char* argv[])
{
	...
	{
		SWAP									// do context switch
		int base = (*argv[i] == '0' && *(argv[i] + 1) == 'x') ? 16 : 10;
		long val = strtol(argv[i], &ptr, base);
		if (*ptr != 0) {
			printf("\nArgument %d invalid. Bad number: %s", i, argv[i]);
			return -1;
		}
		sum += val;
	}
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("args", "args", P1_args, "Print argc and argv");
	commands[i++] = newCommand("add", "add", P1_add, "Add the parameters given in decimal or hexadecimal");
